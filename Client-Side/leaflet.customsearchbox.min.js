...
function getControlHrmlContent() {
    return '<div id="controlbox" > ' +
                '<div id="boxcontainer" class="searchbox searchbox-shadow" > ' +
                    ...
                    ' </div> ' +
                    ' <div> ' +
                        ' <input id="searchboxinput" type="text" value="King" oninput="detectInputChange(true)" style="position: relative;"/> ' +
                    ' </div> ' +
                        ...
                        ' <span id="panel-header-title" class="panel-header-title"> ' +
                            '<div id="controlboxInner" > ' +
                                '<div id="boxcontainerInner" class="searchbox searchbox-shadow" > ' +
                                    ...
                                    ' </div> ' +
                                    ' <div> ' +
                                        ' <input id="searchboxinputInner" type="text" value="King"  oninput="detectInputChange(false)" style="position: relative;"/> ' +
                                    ' </div> ' +
                                    ' <div class="searchbox-searchbutton-container"> ' +
                            ...
                            ' </div> ' +
                        ' </span> ' +
                        ' <button aria-label="Menu" id="panelbutton" class="panel-close-button" onclick="toggleOutterSearchBox();"></button> ' +
                    ' </div> ' +
                ' </div> ' +
...
function generateHtmlContent(a) {
    ...
    /*b +=' <li class="panel-list-item d-flex justify-content-center pt-0 pb-2">' +
            ' <div class="d-flex" justify-content-center">' +
                ' <input type="radio" name="scale" id="nation" value="nation"> Nation <br>' +
                ' <input type="radio" name="scale" id="state" value="state"> State <br>' +
                ' <input type="radio" name="scale" id="county" value="county"> County <br>' +
                ' <input type="radio" name="scale" id="city" value="city"> City <br>' +
            ' </div>' +
          '</li>'*/
    ...
    b +=' <li class="panel-list-item d-flex justify-content-center pt-0 pb-2">' +
            ' <div class="d-flex justify-content-center">' +
                ' <select id="scale">' +
                  ...
                  '<option id="4" value="city"> City </option>' +
                '</select>' +
            ' </div>' +
          '</li>'
    ...
    for (d = 0; d < a.Items.length; d++) {
        ...
        //Add checkbox
        "checkbox" == c.type && (b += '<span class="panel-list-item-icon ' + c.icon + '" ></span>', b += '<input type=checkbox id="' + c.value + 'ID" onclick="' + c.onclick + '" value="' + c.value + '"> <span class="text-white pl-3">' + c.name + '</span>') ;
        b += "</li></div>"
        ...
        b += '<li class="panel-list-item pt-0 pb-0" style="display:none;"> ' +
                ...
                ' <br/>' +
                ' <div>' +
                    ' <p id="value' + c.value + '" class="text-center text-white mb-0"> Value: ' + c.min + ' </p>' +
                ' </div>'
            ' </li>'
...
//function to add searchbar into searchbox control title text
