// Full Credit goes to https://github.com/8to5Developer
// 
// 
...
function generateHtmlContent(a) { 
    for (var b = '<ul class="panel-list">', d = 0; d < a.Items.length; d++) { 
        ...
        //Add checkbox
        "checkbox" == c.type && (b += '<span class="panel-list-item-icon ' + c.icon + '" ></span>', b += '<input type=checkbox id="' + c.value + 'ID" onclick="' + c.onclick + '" value="' + c.value + '">' + c.name) ;
        b += "</li></div>"
        
        //Add slider
        b += '<li class="panel-list-item" style="display:none;"> ' + 
                ' <div class="row"> ' + 
                    ' <div class="col pr-0 text-center">' + 
                        c.min +
                    '  </div> ' + 
                    ' <div class="col-8 px-0"> ' + 
                        ' <input id="' + c.value + 'SliderID" type="range" class="custom-range" min="' + c.min +'" max="' + c.max + '" onchange="updateSliderValue(' + "'" + '' + c.value + "'" + ', this.value);" > ' + 
                    ' </div> ' + 
    ...
    //Add Submit button
    b += '<li class="panel-list-item">' +
            '<button type="button" class="m-*-auto" onclick="submitValues()" >' +
                'Submit' +
            '</button>' +
        ' </li>'
    return b + "</ul>" 
}
////////////////////////Line 12 needs to add checkbox
...
function createSearchboxControl() {
    return L.Control.extend({
        ...
        initialize: function (a) { L.Util.setOptions(this, a); a.sidebarTitleText && (this._sideBarHeaderTitle = a.sidebarTitleText); a.sidebarMenuItems && (this._sideBarMenuItems = a.sidebarMenuItems) }, onAdd: function (a) {
