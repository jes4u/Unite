...
// Editors: Jesse Tran, Jeewon Lee, Seohyung Lee
                ...
                ' <div class="panel-header"> '+
                    ' <div class="panel-header-container"> ' +
                        ' <span id="panel-header-title" class="panel-header-title"> ' +
                            '<div id="panel-title" class="hidden"><p id="panel-header-text" class="panel-header-title">' +
                                'Compare' +
                            '</p></div>' +
...
function generateHtmlContent(a) {
    ...
    if ( a.Items[0].type == "checkbox" ) {
        b +=' <li class="panel-list-item d-flex justify-content-center pt-0 pb-2">' +
            ' <div class="divSelectSearch d-flex justify-content-center">' +
                // These are the variables for the scale dropdown
                ' <select id="scale" class="py-1">' +
                  '<option id="1" value="nation"> Nation </option>' +
                  '<option id="2" value="state"> State </option>' +
                  '<option id="3" value="county"> County </option>' +
                  '<option id="4" value="urbanextent"> City </option>' +
                '</select>' +
            ' </div>' +
          '</li>'
        ...
        b +=' <li class="panel-list-item d-flex justify-content-center pt-0 pb-2">' +
                ' <div class="divSelectSearch d-flex justify-content-center">' +
                    ' <select id="dropDown" class="py-1"> </select>' +
                ' </div>' +
            '</li>'
    }
    ...
    for (d = 0; d < a.Items.length; d++) {
        ...
        b = b + '<li class="panel-list-item pb-2"><div>';

        "search" == c.type ? (b += '<div class="autocomplete"><input id="' + c.id + '" class="compareSearch bg-white py-1" type="text" placeholder="Select Location ' + (d + 1) + '"> </div>') :
        //Add checkbox
        "checkbox" == c.type && (b += '<span class="panel-list-item-icon ' + c.icon + '" ></span>', b += '<input type=checkbox id="' + c.value + 'ID" onclick="' + c.onclick + '" value="' + c.value + '"> <span class="pl-3" title="information">' + c.name + '</span>') ;

        if(d == 0) {
          b += '<img id="info" onclick="viewInfo()" src="./image/information.png" alt="Additional Information" style="width:18px;height:18px;float:right;">' +
               '<div id="myModal" class="modalHelp modalContent">' +
    ...
    var functionCall;
    if (a.Items[0].type == "checkbox") {
        functionCall = "searchPopulationPercent()"
    } else {
        functionCall = "searchCompare()"
    }

    b += '<li class="panel-list-item d-flex justify-content-center pt-4">' +
            '<button type="button" class="m-*-auto btn-light px-4" onclick="' + functionCall + '" >' +
                'Search for Similar Places' +
            '</button>' +
        ' </li>'

    b += '<li class="panel-list-item d-flex justify-content-center pt-4">' +
            "<div>City: &nbsp; </div>" + '<img src="https://cdn.rawgit.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-violet.png" width="15" height="25" margin-right="40" ><br>' + "<div> &nbsp; </div>" +
            "<div>County: &nbsp; </div>" + '<img src="https://cdn.rawgit.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-yellow.png" width="15" height="25" margin-right="40" ><br>' +  "<div> &nbsp; </div>" +
            "<div>State:  &nbsp;</div>" + '<img src="https://cdn.rawgit.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-green.png" width="15" height="25" margin-right="40" ><br>' + "<div> &nbsp; </div>" +
            "<div>Country: &nbsp; </div>" + '<img src="https://cdn.rawgit.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-blue.png" width="15" height="25" margin-right="40" ><br>' + "<div> &nbsp; </div>" +
        ' </li>'
    return b + "</ul>"
}
